...
class ModelModule(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss *= batch_sizes.numel() / batch_sizes.sum()  # world size / total batch size
    ...
    def save_predictions_to_csv(self):
        # 确保输出目录存在
        output_dir = os.path.dirname(self.args.output_csv)
        if output_dir and not os.path.exists(output_dir):
            os.makedirs(output_dir)

        # 将预测结果转换为 DataFrame
        df = pd.DataFrame(self.test_predictions)
        df.index.name = 'index'  # 设置索引名称为 'index'

        # 保存到 CSV 文件
        df.to_csv(self.args.output_csv, index=True)
        print(f"预测结果已保存至 {os.path.abspath(self.args.output_csv)}")
